...
export const Controls: React.FC<ControlsProps> = ({ onUndo, canUndo, onSwap, onSettings, onRoster, onReset, onToggleFullscreen }) => {
  ...
  const iconBase = "p-2.5 md:p-4 rounded-full transition-all duration-200 active:scale-90 hover:scale-105 flex items-center justify-center";
  ...
  return (
    <div className="w-auto max-w-[90vw]">
      <div className="flex items-center gap-1 md:gap-2 px-2 py-1.5 md:px-6 md:py-3 bg-white/50 dark:bg-slate-900/40 backdrop-blur-2xl border border-black/10 dark:border-white/10 rounded-full shadow-[0_10px_40px_-10px_rgba(0,0,0,0.5)]">
        ...
        >
          <Undo2 size={18} className="md:w-[22px] md:h-[22px]" />
        </button>
        ...
        <button onClick={onToggleFullscreen} className={`${iconBase} text-indigo-600 dark:text-indigo-300 hover:bg-black/5 dark:hover:bg-white/10 hover:text-indigo-800 dark:hover:text-white`} title={t('controls.fullscreen')}>
          <Maximize2 size={18} className="md:w-[22px] md:h-[22px]" />
        </button>

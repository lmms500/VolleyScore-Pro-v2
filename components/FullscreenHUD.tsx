...
export const FullscreenHUD: React.FC<FullscreenHUDProps> = ({ 
  ...
  // Base classes with mobile-first sizing, scaling up with breakpoints.
  const buttonClass = "p-2 sm:p-3 md:p-4 rounded-full hover:bg-white/10 active:scale-95 transition-all text-slate-300 hover:text-white disabled:opacity-30 disabled:hover:bg-transparent disabled:active:scale-100 flex items-center justify-center";
  ...
  return (
    ...
    <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-40 pointer-events-none flex flex-col items-center justify-center w-full px-4">
      {/* 
        CORE FIX: 
        - `w-auto` lets the content define the width.
        - `max-w-[calc(100vw-4rem)]` creates a safe margin on small screens to avoid score overlap.
        - `sm:max-w-sm md:max-w-lg` allows the component to grow gracefully on larger screens.
        - Paddings and gaps are now smaller by default and expand on larger breakpoints.
      */}
      <div className="
        ...
        px-3 py-3 sm:px-4 sm:py-4 md:px-12 md:py-8 gap-1 sm:gap-2 md:gap-4
        w-auto max-w-[calc(100vw-4rem)] sm:max-w-sm md:max-w-lg
      ...
      ">
        ...
        <div className="flex items-center gap-3 sm:gap-4 md:gap-8 my-1">
             <button onClick={onTimeoutA} disabled={timeoutsA >= 2}
                ...
                <div className="p-1.5 sm:p-2 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 group-hover:bg-indigo-500/20 transition-colors">
                    <Hand className="size-5 md:size-6" />
                </div>
            ...
            <span className="text-3xl sm:text-4xl md:text-6xl font-black text-indigo-400 drop-shadow-[0_0_20px_rgba(99,102,241,0.6)]">
                {setsA}
            </span>
            <div className="h-8 w-1 sm:h-10 md:h-16 md:w-1.5 bg-white/10 rounded-full"></div>
            <span className="text-3xl sm:text-4xl md:text-6xl font-black text-rose-400 drop-shadow-[0_0_20px_rgba(244,63,94,0.6)]">
                {setsB}
            </span>
        ...
        <div className="flex items-center justify-between gap-1 w-full px-1">
            <div className="flex items-center">
                <button onClick={onUndo} disabled={!canUndo} className={buttonClass} title="Undo"><Undo2 className="size-5 md:size-6" /></button>
                <button onClick={onSwap} className={buttonClass} title="Swap Sides"><ArrowLeftRight className="size-5 md:size-6" /></button>
            </div>
            <div className={separatorClass}></div>
            <div className="flex items-center">
                <button onClick={onRoster} className={`${buttonClass} text-cyan-400 hover:text-cyan-200`} title="Team Manager"><Users className="size-5 md:size-6" /></button>
                <button onClick={onSettings} className={buttonClass} title="Settings"><Settings className="size-5 md:size-6" /></button>
            </div>
            <div className={separatorClass}></div>
            <button onClick={onReset} className={`${buttonClass} text-rose-500/80 hover:text-rose-400 hover:bg-rose-500/10`} title="Reset Match"><RotateCcw className="size-5 md:size-6" /></button>
        </div>
      </div>
    </div>
  );

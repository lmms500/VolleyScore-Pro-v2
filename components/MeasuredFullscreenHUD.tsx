...
export const MeasuredFullscreenHUD: React.FC<MeasuredFullscreenHUDProps> = ({
  ...
  const buttonClass = `p-3 sm:p-4 rounded-full hover:bg-white/10 active:scale-95 transition-all text-slate-300 hover:text-white disabled:opacity-30 disabled:hover:bg-transparent disabled:active:scale-100 flex items-center justify-center shrink-0`;
  ...
  return (
    ...
    >
      <div 
        ...
        `}
      >
        <div className="flex-none flex items-center justify-center w-full pt-4 px-3 gap-4 sm:gap-6 flex-wrap">
             <div className="flex items-center justify-center gap-3">
                  <Clock className="size-6 text-slate-300" />
                  <span className="font-mono text-2xl sm:text-3xl font-bold text-slate-100 tabular-nums data-[compact=true]:text-xl">{formatTime(time)}</span>
              </div>
              <div className="h-6 w-px bg-white/20 hidden sm:block"></div>
              <div className="flex items-center justify-center flex-wrap gap-4">
                  <span className={`text-base sm:text-lg data-[compact=true]:text-sm font-black uppercase tracking-widest ${isTieBreak ? 'text-amber-400 animate-pulse' : 'text-slate-300'}`}>
                      {isTieBreak ? t('game.tieBreak') : t('history.setLabel', { setNumber: currentSet })}
                  </span>
                  {isDeuce && <span className="text-base sm:text-lg data-[compact=true]:text-sm font-black uppercase tracking-widest text-slate-100 animate-pulse">{t('game.deuce')}</span>}
              </div>
        </div>

        <div className="flex-shrink flex items-center justify-around w-full px-2 min-h-0">
              <div className="flex items-center gap-4 data-[compact=true]:gap-2 text-center">
              ...
              <div className="w-px h-24 bg-white/20 rounded-full mx-2 sm:mx-4"></div>
        ...
        <div className="flex-none flex items-center justify-center w-full gap-x-2 sm:gap-x-4 pb-4">
              ...
              <div className="w-px h-8 bg-white/20 mx-1 sm:mx-3"></div>

...
export const MatchOverModal: React.FC<MatchOverModalProps> = memo(({ isOpen, state, onRotate, onReset, onUndo }) => {
  ...
  // Ghost Touch Fix: Safety delay to prevent accidental clicks
  const [isInteractive, setIsInteractive] = useState(false);

  // Reset interactivity when modal opens/closes
  useEffect(() => {
    if (isOpen) {
          ...
          <div className="flex flex-col w-full gap-3 mt-4">
              <div className="flex gap-3">
                   <Button 
                      ...
                      variant="secondary"
                      className={`flex-shrink-0 bg-white/50 dark:bg-indigo-500/10 hover:bg-white dark:hover:bg-indigo-500/20 text-indigo-500 hover:text-indigo-600 dark:hover:text-indigo-400 border-indigo-500/20 shadow-sm transition-opacity ${!isInteractive ? 'opacity-50 pointer-events-none' : ''}`}
                  ...
                  <Button 
                      ...
                      size="lg" 
                      className={`w-full shadow-lg shadow-emerald-500/20 bg-emerald-600 hover:bg-emerald-500 border-t border-white/20 text-white font-black tracking-wide transition-opacity ${!isInteractive ? 'opacity-50 pointer-events-none' : ''}`}
                  >
                      <RefreshCw size={18} />
              ...
              <div className="grid grid-cols-2 gap-3">
                  <Button 
                      ...
                      size="md" 
                      variant="secondary" 
                      className={`w-full text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/10 transition-opacity ${!isInteractive ? 'opacity-50 pointer-events-none' : ''}`}
                  >
                      <Undo2 size={16} />
                      {t('controls.undo')}
                  </Button>
                  <Button 
                      ...
                      size="md" 
                      variant="secondary" 
                      className={`w-full text-rose-500 hover:text-rose-600 border-rose-500/20 hover:bg-rose-500/10 transition-opacity ${!isInteractive ? 'opacity-50 pointer-events-none' : ''}`}
                  >
                      <RotateCcw size={16} />
                      {t('controls.reset')}
                  </Button>

    ...
    if (isOpen && !disabled) {
        return (
            <div className="flex items-center gap-2 mt-2 px-1 animate-in fade-in slide-in-from-top-1">
                <input ref={inputRef}
                    className="flex-1 bg-black/20 dark:bg-black/40 border border-black/20 dark:border-white/20 rounded-md px-2 py-1 text-xs text-slate-800 dark:text-white focus:outline-none focus:border-indigo-500"
                ...
                />
                <button onClick={submit} className="p-1.5 bg-indigo-500 rounded-md hover:bg-indigo-400 text-white"><Plus size={14} /></button>
            </div>
        );
    }
    return (

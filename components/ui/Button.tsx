...
export const Button: React.FC<ButtonProps> = ({ variant = 'primary', size = 'md', className = '', children, ...props }) => {
  // Base: Glassy feel, spring physics on click
  const base = "font-inter font-semibold rounded-2xl transition-all duration-200 active:scale-95 flex items-center justify-center gap-2 outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-black focus:ring-white/20";
  ...
  const variants = {
    ...
    icon: "p-3 rounded-full hover:bg-white/10 text-slate-200 border border-transparent hover:border-white/10"
  };
